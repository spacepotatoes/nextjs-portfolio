...
export default function AdminPage() {
  ...
  // WICHTIG: Wenn wir editieren, muss die bestehende Bild-URL in den State
  useEffect(() => {
    if (projectToEdit) {
      setImageUrl(projectToEdit.imageUrl || "");
    } else {
      setImageUrl(""); // Reset wenn nicht im Edit-Modus
    }
  }, [projectToEdit]);
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <section className="bg-zinc-900/40 p-8 rounded-2xl border border-zinc-800 backdrop-blur-md">
          <h2 className="text-2xl font-bold mb-6 text-purple-400">
            {projectToEdit ? `✏️ "${projectToEdit.title}" bearbeiten` : "🚀 Neues Projekt"}
          </h2>
          
          <form 
            action={async (formData) => {
              // Das Bild aus dem State an das Formular hängen
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {imageUrl && (
                <div className="relative w-32 h-20 group">
                  ...
                  <button onClick={() => setImageUrl("")} className="absolute -top-2 -right-2 bg-red-500 rounded-full w-5 h-5 text-[10px] hidden group-hover:flex items-center justify-center">✕</button>
                </div>
              )}
            </div>
            ...
            {/* NEU: Tech Stack & Design Tools */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input 
                name="techStack" 
                defaultValue={projectToEdit?.techStack?.join(", ") || ""} 
                placeholder="Tech Stack (z.B. Next.js, React, Prisma)" 
                className="p-3 rounded-xl bg-black border border-zinc-800 outline-none focus:border-purple-500" 
              />
              <input 
                name="designTools" 
                defaultValue={projectToEdit?.designTools?.join(", ") || ""} 
                placeholder="Design Tools (z.B. Figma, Spline)" 
                className="p-3 rounded-xl bg-black border border-zinc-800 outline-none focus:border-purple-500" 
              />
            </div>
